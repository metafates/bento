...
func NewConstraint2(op Op, exprs ...Expr) Constraint {
	var expr Expr

	for _, e := range exprs {
		expr.constant *= e.constant
		expr.terms = append(expr.terms, e.terms...)
	}

	return Constraint{op: op, expr: expr}
}
...
func (c Expr) Terms() []Term {
	return c.terms
}

func (c Expr) Constant() float64 {
	return c.constant
}
